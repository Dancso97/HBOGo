...
def CATEGORIES():
	...
	req = urllib2.Request('https://huapi.hbogo.eu/v7/Groups/json/HUN/ANMO/0/True', None, loggedin_headers)
	opener = urllib2.build_opener()
	f = opener.open(req)
	jsonrsp = json.loads(f.read())
	...
	try:
		...
		pass
	for cat in range(0, 3): #4. fake dir
		addDir(jsonrsp['Items'][cat]['Name'].encode('utf-8', 'ignore'),jsonrsp['Items'][cat]['ObjectUrl'],'',1,md+'DefaultFolder.png')
		addDir(jsonrsp['Items'][3]['Name'].encode('utf-8', 'ignore'),'http://huapi.hbogo.eu/v7/Group/json/HUN/ANMO/57d2ad80-9551-4843-a905-3d33fc289900/0/0/0/0/0/0/True','',1,md+'DefaultFolder.png')
def LIST(url):
	global sessionId
	global loggedin_headers
	...
	if len(jsonrsp['Container']) > 1:
		...
		for titles in range(0, len(jsonrsp['Container'][0]['Contents']['Items'])):
			if jsonrsp['Container'][0]['Contents']['Items'][titles]['ContentType'] == 1: #1=MOVIE/EXTRAS, 2=SERIES(serial), 3=SERIES(episode)
				...
				#If it's a serial episode   
				plot = jsonrsp['Container'][0]['Contents']['Items'][titles]['Abstract'].encode('utf-8', 'ignore')
				addLink(jsonrsp['Container'][0]['Contents']['Items'][titles]['ObjectUrl'],plot,jsonrsp['Container'][0]['Contents']['Items'][titles]['AgeRating'],jsonrsp['Container'][0]['Contents']['Items'][titles]['ImdbRate'],jsonrsp['Container'][0]['Contents']['Items'][titles]['BackgroundUrl'],[jsonrsp['Container'][0]['Contents']['Items'][titles]['Cast'].split(', ')][0],jsonrsp['Container'][0]['Contents']['Items'][titles]['Director'],jsonrsp['Container'][0]['Contents']['Items'][titles]['Writer'],jsonrsp['Container'][0]['Contents']['Items'][titles]['Duration'],jsonrsp['Container'][0]['Contents']['Items'][titles]['Genre'],jsonrsp['Container'][0]['Contents']['Items'][titles]['SeriesName'].encode('utf-8', 'ignore')+' SEASON '+str(jsonrsp['Container'][0]['Contents']['Items'][titles]['SeasonIndex'])+' EPISODE '+str(jsonrsp['Container'][0]['Contents']['Items'][titles]['Index']),jsonrsp['Container'][0]['Contents']['Items'][titles]['OriginalName'],jsonrsp['Container'][0]['Contents']['Items'][titles]['ProductionYear'],5)
			else:
				# If it's a series
...
def EPISODE(url):
	...
	for episode in range(0, len(jsonrsp['ChildContents']['Items'])):
		...
		addLink(jsonrsp['ChildContents']['Items'][episode]['ObjectUrl'],plot,jsonrsp['ChildContents']['Items'][episode]['AgeRating'],jsonrsp['ChildContents']['Items'][episode]['ImdbRate'],jsonrsp['ChildContents']['Items'][episode]['BackgroundUrl'],[jsonrsp['ChildContents']['Items'][episode]['Cast'].split(', ')][0],jsonrsp['ChildContents']['Items'][episode]['Director'],jsonrsp['ChildContents']['Items'][episode]['Writer'],jsonrsp['ChildContents']['Items'][episode]['Duration'],jsonrsp['ChildContents']['Items'][episode]['Genre'],jsonrsp['ChildContents']['Items'][episode]['SeriesName'].encode('utf-8', 'ignore')+' SEASON '+str(jsonrsp['ChildContents']['Items'][episode]['SeasonIndex'])+' '+jsonrsp['ChildContents']['Items'][episode]['Name'].encode('utf-8', 'ignore'),jsonrsp['ChildContents']['Items'][episode]['OriginalName'],jsonrsp['ChildContents']['Items'][episode]['ProductionYear'],5)
...
def PLAY(url):
	...
	# Logging in if the user is anonymous

	if sessionId == '00000000-0000-0000-0000-000000000000':
		LOGIN()

	# Providing external / alternative subtitles
	if se=='true':
		try:
			req = urllib2.Request('http://huapi.hbogo.eu/v7/Content/json/HUN/ANMO/'+cid, None, loggedin_headers)
			print(cid)
			...
			# Download the official TTML episode subtitles and convert them to SRT
			try:
				...
				# Generates an error to exclude subtitles for any problem
				if sub != 'true':
					raise Exception()
	...
	req = urllib2.Request('https://huapi.hbogo.eu/v7/Purchase/Json/HUN/COMP', purchase_payload, purchase_headers)
	opener = urllib2.build_opener()
	f = opener.open(req)
	...
	if is_helper.check_inputstream():
		...
		if (se=='true' and sub=='true'): # Set external subtitles if this mode is selected
...
def SEARCH():
	keyb = xbmc.Keyboard(search_string, 'Searching for movies, series ...')
	keyb.doModal()
	searchText = ''
	if (keyb.isConfirmed()):
		searchText = urllib.quote_plus(keyb.getText())
		if searchText == "":
			addDir('Go back - there are no results matching the search','','','',md+'DefaultFolderBack.png')
		else:
			# Save the search query to the settings
			__settings__.setSetting('lastsearch', searchText)
			# Perform Search
			req = urllib2.Request('https://huapi.hbogo.eu/v7/Search/Json/HUN/ANMO/'+searchText.decode('utf-8', 'ignore').encode('utf-8', 'ignore')+'/0/0/0/0/0/1', None, loggedin_headers)
			opener = urllib2.build_opener()
			f = opener.open(req)
			jsonrsp = json.loads(f.read())        
...
# Module to add a separate directory and its attributes to the contents of the Kodi directory shown - NO NEED TO CHANGE HERE
...
#List of individual sub-programs / modules in this plugin - must fully match the code above
